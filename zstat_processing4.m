...
zstatmeans_final_table= array2table(Tablezstatmeans,'RowNames',rowNames,'VariableNames',Headers)
writetable(zstatmeans_final_table, sprintf('%s/zstatmeans_final_table_%s.csv',FiguresOut,(datestr(clock,'yyyy-mm-dd_HH-MM-SS'))))

% zstatmeansNOZ %
TablezstatmeansNOZ = cell2mat(zstatmeansNOZ_final)
zstatmeansNOZ_final_table= array2table(TablezstatmeansNOZ,'RowNames',rowNames,'VariableNames',Headers) 
writetable(zstatmeansNOZ_final_table, sprintf('%s/zstatmeansNOZ_final_table_%s.csv',FiguresOut,(datestr(clock,'yyyy-mm-dd_HH-MM-SS'))))

% zstatmedians %
Tablezstatmedians = cell2mat(zstatmedians_final)
zstatmedians_final_table= array2table(Tablezstatmedians,'RowNames',rowNames,'VariableNames',Headers)
writetable(zstatmedians_final_table, sprintf('%s/zstatmedians_final_table_%s.csv',FiguresOut,(datestr(clock,'yyyy-mm-dd_HH-MM-SS'))))
%zstatmediansNOZ
TablezstatmediansNOZ = cell2mat(zstatmediansNOZ_final)
zstatmediansNOZ_final_table= array2table(TablezstatmediansNOZ,'RowNames',rowNames,'VariableNames',Headers) 
writetable(zstatmediansNOZ_final_table, sprintf('%s/zstatmediansNOZ_final_table_%s.csv',FiguresOut,(datestr(clock,'yyyy-mm-dd_HH-MM-SS'))))
%save('zstatmediansNOZ_final_table.csv', fullfile(FiguresOut,['zstatmediansNOZ_final_table_' datestr(clock,'yyyy-mm-dd_HH-MM-SS') '.csv'])); %save
...
%% Compare young to old, by MRI Param
%all
tmp2_TablezstatmediansNOZ{1} = TablezstatmediansNOZ(:,1:3:end); 
tmp2_TablezstatmediansNOZ{2} = TablezstatmediansNOZ(:,2:3:end); 
tmp2_TablezstatmediansNOZ{3} = TablezstatmediansNOZ(:,3:3:end); 
%young
tmp2_TablezstatmediansNOZ{4} = TablezstatmediansNOZ(:,1:3:19); %columns 1 to 19 (sub-01 to sub-09)
tmp2_TablezstatmediansNOZ{5} = TablezstatmediansNOZ(:,2:3:20); 
tmp2_TablezstatmediansNOZ{6} = TablezstatmediansNOZ(:,3:3:21); 
%old
tmp2_TablezstatmediansNOZ{7} = TablezstatmediansNOZ(:,22:3:end);  %columns 22 to end (sub-10 to sub-14)
tmp2_TablezstatmediansNOZ{8} = TablezstatmediansNOZ(:,23:3:end); 
tmp2_TablezstatmediansNOZ{9} = TablezstatmediansNOZ(:,24:3:end); 
tmp2=tmp2_TablezstatmediansNOZ{1:9}
y = num2cell(1:numel(tmp2_TablezstatmediansNOZ)); 
x = cellfun(@(x, y) [x(:) y*ones(size(x(:)))], tmp2_TablezstatmediansNOZ, y, 'UniformOutput', 0); % adding labels to the cells 
X = vertcat(x{:}); 
figure;
boxplot(X(:,1), X(:,2), 'Labels',{'allDefault','allparamA', 'allparamB','youngDefault','youngparamA', 'youngparamB','oldDefault','oldparamA', 'oldparamB'})
%plot(tmp2_TablezstatmediansNOZ{1},'Color','c')
title('Zstat Medians NOZ Compare Young vs. Old')
set(findobj(gca,'type','line'),'linew',3)
set(gca,'linew',2)
xCenter = 1:numel(tmp2); 
spread = 0.5; % 0=no spread; 0.5=random spread within box bounds (can be any value)
for i = 1:numel(tmp2)
    plot(rand(size(tmp2{i}))*spread -(spread/2) + xCenter(i), tmp2{i}, 'mo','linewidth', 1)
end
...
tmp_TablezstatmediansNOZ{2} = TablezstatmediansNOZ(:,2:3:end); 
tmp_TablezstatmediansNOZ{3} = TablezstatmediansNOZ(:,3:3:end); 
allData = {tmp_TablezstatmediansNOZ{1}; tmp_TablezstatmediansNOZ{2}; tmp_TablezstatmediansNOZ{3}}; 
y = num2cell(1:numel(tmp_TablezstatmediansNOZ)); 
x = cellfun(@(x, y) [x(:) y*ones(size(x(:)))], tmp_TablezstatmediansNOZ, y, 'UniformOutput', 0); % adding labels to the cells 
X = vertcat(x{:}); 
figure;
...
%plot(1,X,'o')
%plot(X(:,1), 'o', X(:,2), 'x')
title('zstatmediansNOZ single subjects')
%saveas(gcf,fullfile(FiguresOut,['zstatmediansNOZ_boxplot_allsubjs' datestr(clock,'yyyy-mm-dd_HH-MM-SS') '.jpg'])); %save

%% sandpit - edit with data points showing - Zstat2 medians NOZ Individual Subjects
tmp_TablezstatmediansNOZ{1} = TablezstatmediansNOZ(:,1:3:end); 
tmp_TablezstatmediansNOZ{2} = TablezstatmediansNOZ(:,2:3:end); 
tmp_TablezstatmediansNOZ{3} = TablezstatmediansNOZ(:,3:3:end); 
allData = {tmp_TablezstatmediansNOZ{1}; tmp_TablezstatmediansNOZ{2}; tmp_TablezstatmediansNOZ{3}}; 
...
set(findobj(gca,'type','line'),'linew',3)
set(gca,'linew',2)
title('Zstat2 medians NOZ Individual Subjects')
xCenter = 1:numel(allData); 
spread = 0.5; % 0=no spread; 0.5=random spread within box bounds (can be any value)
for i = 1:numel(allData)
    plot(rand(size(allData{i}))*spread -(spread/2) + xCenter(i), allData{i}, 'mo','linewidth', 1)
end

set(h,'Marker','o'); % Change symbols for all the groups. 
set(h(1),'MarkerEdgeColor','g'); % Change color for one group 



%inputdata
Data1=tmp_TablezstatmediansNOZ{1}
Data2=tmp_TablezstatmediansNOZ{2}
Data3=tmp_TablezstatmediansNOZ{3}
