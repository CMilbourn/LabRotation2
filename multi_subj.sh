...
if [ $# -lt 1 ]; then
  echo "Usage: $0 <sourcedata folder> <SubjectNumber> <MRIParameter> e.g [default] (default,paramA,paramB) [options] Output: .Feat Full analysis folder. e.g. ./multi_subj.sh /Users/colette/sourcedata/ sub-01 paramA gives sub-01_FullparamA.feat"; 
 #exit 1 ;
fi
...
mkdir ${src}/derivatives/ #if the directory does not exist, then make a folder called 'derivatives'
fi
...
#Copy the template feat design.fsf file and put one called design{MRIParam} in the source out folder
