...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%subject List
numlist = {2,3}; %Creates a list with the subject numbers to run through called 'numlist'

for a = 1:length(numlist) %start of for loop - runs through one number in 'numlist' per iteration
%% Variables
subj=numlist{a}

sub_folder= sprintf('%s/sub-0%d/CVR/', src, subj) %subject CVR sub_folder

%Load breath by breath file%
bbb_mod= sprintf('%s/sub-0%d_bbb_mod.txt', sub_folder,subj) %subj bbb_mod.txt file - breath by breath file path
...
eventsRobustCVR= sprintf('%s/sub-0%d_events_RobustCVR.txt', sub_folder,subj) %subj eventsRobustCVR.txt file - timings of default, paramA, parmaB events
...
%% Load Events file
eventsfile= sprintf('%s/sub-0%d_events_start.csv', sub_folder,subj);
